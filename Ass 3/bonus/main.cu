...
// record the unused times of each page entry, 1024 page entries
__device__ __managed__ u32 page_counter[1024] = {0};
...
__global__ void mykernel(int input_size)
{
  ...
  VirtualMemory vm;
  vm_init(&vm, data, storage, pt, &pagefault_num, page_counter, PAGE_SIZE,
